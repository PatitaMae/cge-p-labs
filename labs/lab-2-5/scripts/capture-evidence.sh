...
( cd "$WORKSPACE" && [[ -f tfplan ]] && \
    /c/terraform/terraform.exe show -json tfplan > "$BUNDLE_DIR/plan.json" 2>/dev/null || true )
( cd "$WORKSPACE" && /c/terraform/terraform.exe state pull > "$BUNDLE_DIR/state.json" 2>/dev/null || true )
( cd "$WORKSPACE" && git log -1 --pretty=full > "$BUNDLE_DIR/commit.txt" 2>/dev/null \
    || echo "no git commit available" > "$BUNDLE_DIR/commit.txt" )
/c/terraform/terraform.exe version > "$BUNDLE_DIR/version.txt"
