...
( cd "$WORKSPACE" && /c/terraform/terraform.exe show -json tfplan > "$WORKSPACE/plan.json" )
...
EXIT=0
{
  ...
  for ns in compliance.sc28_aws compliance.ac3_aws compliance.cm6_aws compliance.cm6 ; do
    ...
    if echo "$OUT" | py -c 'import sys,json; d=json.load(sys.stdin); sys.exit(0 if all(len(r.get("failures") or [])==0 for r in d) else 1)'; then : ; else EXIT=1 ; fi
    echo "$OUT"
  done
  echo "]"
